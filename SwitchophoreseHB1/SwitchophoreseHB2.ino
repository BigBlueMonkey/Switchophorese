...
int initSec = 0; // initial timer
int initMin = 0;
int initHour = 0;

int runSec = 0;// run time
int runMin = 0;
int runHour = 0;
...
if ( currentMillis-startInit < (initSec*1000+initMin*60000+initHour*3600000)+elapsedInitTime && pause == false && isTimer == true){                                                     //init Timer
    ...
    int timeInitText = (initSec+initMin*60+initHour*3600+elapsedInitTime/1000)-(currentMillis-startInit)/1000;
    int hourInitText = timeInitText / 3600;
...
if (pulsebMillis-previousbPulseMillis >= Tsb +(incrementExpo*count)*ratio/100){
  ...
  if (runMillis - previousMillis >= 1000 ) {    // Run timer
    ...
    int timeText = (runSec+runMin*60+runHour*3600+elapsedRunTime/1000)-(runMillis-startRun)/1000;
    int hourText = timeText / 3600;
